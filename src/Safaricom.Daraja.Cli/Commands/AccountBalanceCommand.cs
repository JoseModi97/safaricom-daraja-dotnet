...
public static class AccountBalanceCommand
{
    private const string Usage =
        "Usage: safaricom-daraja account-balance --initiator <name> --shortcode <code> --remarks <text> " +
        "--result-url <url> --timeout-url <url> (+ SecurityCredential flags - see --help)";

    public static async Task ExecuteAsync(string[] args)
    {
        var flags = CliFlags.Parse(args);
        var config = CliConfigLoader.LoadConfig();
        var client = new AccountBalanceClient(new DarajaTransport(config));
        ...
        try
        {
            var response = await client.QueryAsync(new AccountBalanceRequest
            {
                Initiator = initiator,
